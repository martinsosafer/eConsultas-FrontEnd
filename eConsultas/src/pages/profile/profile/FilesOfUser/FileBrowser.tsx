...
export const FilesBrowser = ({ email }: { email: string }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const filteredFiles = filesResponse.filter(
          (f) =>
            f.name !== "PROFILE_PICTURE" &&
            !f.type.includes("profile-picture")
        );

        const filteredTypes = typesResponse.filter(
          (t) => t !== "PROFILE_PICTURE"
        );
  ...
  useEffect(() => {
    if (selectedType === "all") {
      ...
      setFilteredFiles(
        files.filter((f) => f.name.split("-")[0] === selectedType)
      );
    }
  }, [selectedType, files]);
  ...
  const renderFileName = async (fileName: string): Promise<string> => {
    ...
    try {
      const consulta: Consulta = await consultaApi.getConsultaByConsultaId(
        Number(id)
      );
  ...
  return (
    <div className="p-6 h-full flex flex-col gap-4">
      ...
      <ScrollArea className="h-[calc(100vh-180px)] pr-4">
        {/* Ajustamos el contenedor para que sea un 40% más grande (aprox) y agregamos algo de estilo */}
        <div className="space-y-2 max-w-[75rem] mx-auto w-full p-4 bg-gray-50 border border-gray-200 rounded-lg shadow-sm">
          {loading
...
const FileItem = ({
  ...
  return (
    <div
      className="p-4 border rounded-lg cursor-pointer hover:bg-accent/10 transition-colors hover:shadow-md group"
