...
export default function ReportesManagement() {
  ...
  if (!isSuperAdmin)
    return (
      <div className="p-6 text-red-500">Acceso restringido a Super Admin</div>
    );
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const [generalReport, tiposServicio, serviceTypeReport] =
          await Promise.all([
  ...
  const totals = {
    ...
    totalSalaries: generalData.reduce(
      (acc, curr) => acc + curr.salaryExpenses,
      0
    ), // Total gastos en sueldos
    netProfit: generalData.reduce(
      (acc, curr) => acc + (curr.paidAmount - curr.salaryExpenses),
      0
    ), // Ganancia neta
  };
  ...
  return (
    <div className="p-4 md:p-6 space-y-8">
      ...
      <section className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <Button
            ...
            onClick={() => {
              ...
              exportToCSV(
                data,
                `Reporte_General_${new Date().toISOString().split("T")[0]}.csv`
              );
            }}
        ...
        <div className="pt-4">
          <AdvancedBarChart data={generalData} showSalaries={true} />
        </div>
      </section>

      {/* Gráfico de Torta y Detalle por Tipo de Servicio */}
      ...
      <section className="space-y-6 border-t pt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <PersonaSelectorCard
            ...
            onRemove={() =>
              setFilters((prev) => ({ ...prev, patient: undefined }))
            }
          />
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-end">
          ...
          <Button onClick={executeSpecificSearch} disabled={loading.specific}>
            {loading.specific ? (
